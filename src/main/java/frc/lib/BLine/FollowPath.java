...
public class FollowPath extends Command {
    private static final java.util.logging.Logger logger =
            java.util.logging.Logger.getLogger(FollowPath.class.getName());
    ...
    public static void setTranslationListLoggingConsumer(
            Consumer<Pair<String, Translation2d[]>> translationListLoggingConsumer) {
        if (translationListLoggingConsumer == null) {
            return;
        }
    ...
    public static void setBooleanLoggingConsumer(Consumer<Pair<String, Boolean>> booleanLoggingConsumer) {
        if (booleanLoggingConsumer == null) {
            return;
        }
    ...
    private BooleanSupplier externalRotationAtSetpoint = null;
    ...
    private record RotationSelection(int activeRotationIndex, int previousRotationIndex) {}
    ...
    public static class Builder {
        ...
        public FollowPath build(Path path, BooleanSupplier externalRotationAtSetpoint) {
            return new FollowPath(
                    path,
                    driveSubsystem,
                    poseSupplier,
    ...
    private FollowPath(
            ...
            PIDController rotationController,
            PIDController crossTrackController,
            BooleanSupplier externalRotationAtSetpoint) {
        ...
        this.rotationController = rotationController;
        this.crossTrackController = crossTrackController;
        this.externalRotationAtSetpoint = externalRotationAtSetpoint;
    ...
    public void initialize() {
        if (translationController == null || rotationController == null) {
            throw new IllegalArgumentException(
                    "Translation and rotation controllers must be provided and must not be null");
        }
        ...
        lastSpeeds = ChassisSpeeds.fromRobotRelativeSpeeds(
                robotRelativeSpeedsSupplier.get(), pathInitStartPose.getRotation());
        ...
        translationListLoggingConsumer.accept(
                new Pair<>("FollowPath/pathTranslations", pathTranslations.toArray(Translation2d[]::new)));
    }
    ...
    public void execute() {
        ...
        if (rotationSelection.previousRotationIndex() >= 0
                ...
                && pathElementsWithConstraints
                                .get(rotationSelection.previousRotationIndex())
                                .getFirst()
                        instanceof RotationTarget) {
            previousRotationElementTargetRad = ((RotationTarget) pathElementsWithConstraints
                            .get(rotationSelection.previousRotationIndex())
                            .getFirst())
                    .rotation()
                    .getRadians();
        ...
        Translation2d targetTranslation = isTranslationTargetAt(translationElementIndex)
                ? ((TranslationTarget) pathElementsWithConstraints
                                .get(translationElementIndex)
                                .getFirst())
                        .translation()
        ...
        if (!(pathElementsWithConstraints.get(translationElementIndex).getSecond()
                instanceof TranslationTargetConstraint)) {
            logger.warning("FollowPath: Expected TranslationTargetConstraint at index " + translationElementIndex);
            stopCommandedMotion();
            return;
        }
        TranslationTargetConstraint translationConstraint = (TranslationTargetConstraint)
                pathElementsWithConstraints.get(translationElementIndex).getSecond();

        // Clamp translation controller output as to not overpower the crossTrackController output during the velo accel
        // limiting phase
        double translationControllerOutput = MathUtil.clamp(
                -translationController.calculate(remainingDistance, 0),
        ...
        // dont clamp cross track controller as users may prefer to tune their controller to be hyper response to cross
        // track
        ...
        if (rotationSelection.activeRotationIndex() >= 0
                && isRotationTargetAt(rotationSelection.activeRotationIndex())) {

            RotationTarget currentRotationTarget = (RotationTarget) pathElementsWithConstraints
                    .get(rotationSelection.activeRotationIndex())
                    .getFirst();
            if (!(pathElementsWithConstraints
                            .get(rotationSelection.activeRotationIndex())
                            .getSecond()
                    instanceof RotationTargetConstraint)) {
            ...
            if (currentRotationTarget.profiledRotation()) {
                ...
                targetRotationRad =
                        MathUtil.angleModulus(currentRotationTarget.rotation().getRadians());
            }
        ...
        robotRelativeSpeedsConsumer.accept(
        ...
        if (logCounter++ % 3 == 0) {
            ...
            translationListLoggingConsumer.accept(
                    new Pair<>("FollowPath/robotTranslations", robotTranslations.toArray(Translation2d[]::new)));
        }
        ...
        logDouble(
                ...
                Math.toDegrees(currentRotationTargetRad
    ...
    private void advanceTranslationTargets(Pose2d currentPose) {
        while (true) {
            ...
            TranslationTarget currentTranslationTarget = (TranslationTarget)
                    pathElementsWithConstraints.get(translationElementIndex).getFirst();
            double handoffRadius = currentTranslationTarget
    ...
    private TranslationSegmentState getCurrentTranslationSegmentState(Pose2d currentPose) {
        if (translationElementIndex < 0
                || translationElementIndex >= pathElementsWithConstraints.size()
                || !(pathElementsWithConstraints.get(translationElementIndex).getFirst()
                        instanceof TranslationTarget)) {
            Translation2d currentTranslation = currentPose.getTranslation();
            return new TranslationSegmentState(
                    -1, translationElementIndex, currentTranslation, currentTranslation, 0.0, 1.0);
        }
    ...
    private RotationSelection selectRotationTarget(TranslationSegmentState currentSegment) {
        ...
        for (int i = 0; i < pathElementsWithConstraints.size(); i++) {
            ...
            RotationTarget rotationTarget =
                    (RotationTarget) pathElementsWithConstraints.get(i).getFirst();
    ...
    private double getMaxTRatioOnSegment(int segmentEndTranslationIndex) {
        ...
        for (int i = 0; i < pathElementsWithConstraints.size(); i++) {
            ...
            double clampedTRatio = clampTRatio(
                    ((RotationTarget) pathElementsWithConstraints.get(i).getFirst()).t_ratio());
    ...
    private Translation2d getTranslationAtIndex(int translationIndex) {
        if (translationIndex >= 0
                && translationIndex < pathElementsWithConstraints.size()
                && pathElementsWithConstraints.get(translationIndex).getFirst() instanceof TranslationTarget) {
            return ((TranslationTarget)
                            pathElementsWithConstraints.get(translationIndex).getFirst())
                    .translation();
        }
    ...
    private double calculateCrossTrackError() {
        Translation2d targetTranslation = ((TranslationTarget)
                        pathElementsWithConstraints.get(translationElementIndex).getFirst())
                .translation();
        ...
        Translation2d closestPoint =
                calculateProjectedPointOnSegment(prevTranslation, targetTranslation, robotPosition);
    ...
    private Translation2d calculateProjectedPointOnSegment(
        ...
        double dx = segmentEnd.getX() - segmentStart.getX();
        double dy = segmentEnd.getY() - segmentStart.getY();
        return new Translation2d(segmentStart.getX() + t * dx, segmentStart.getY() + t * dy);
    }
    ...
    private Translation2d calculateRotationTargetTranslation(int index) {
        ...
        RotationTarget rotationTarget =
                (RotationTarget) pathElementsWithConstraints.get(index).getFirst();
        double tRatio = clampTRatio(rotationTarget.t_ratio());
        Translation2d pointOnSegment = new Translation2d(
                bounds.startTranslation().getX()
                        + (bounds.endTranslation().getX()
                                        - bounds.startTranslation().getX())
                                * tRatio,
                bounds.startTranslation().getY()
                        + (bounds.endTranslation().getY()
                                        - bounds.startTranslation().getY())
                                * tRatio);
    ...
    private RotationSegmentBounds getRotationSegmentBounds(int rotationIndex) {
        if (rotationIndex < 0
                || rotationIndex >= pathElementsWithConstraints.size()
        ...
        return new RotationSegmentBounds(startTranslationIndex, endTranslationIndex, startTranslation, endTranslation);
    }
    ...
    private void processEventTriggers(Pose2d currentPose) {
        while (eventTriggerElementIndex < pathElementsWithConstraints.size()) {
            PathElement element =
                    pathElementsWithConstraints.get(eventTriggerElementIndex).getFirst();
    ...
    private boolean isEventTriggerTRatioReached(int eventIndex, Pose2d currentPose) {
        ...
        if (isEventTriggerNextSegment(eventIndex)) {
            return false;
        }
        if (isEventTriggerPreviousSegment(eventIndex)) {
            return true;
        }
        ...
        double segmentProgress = calculateSegmentProjectionT(translationA, translationB, currentPose.getTranslation());

        double targetTRatio =
                ((EventTrigger) pathElementsWithConstraints.get(eventIndex).getFirst()).t_ratio();
        return segmentProgress >= targetTRatio;
    }

    private boolean isEventTriggerPreviousSegment(int eventIndex) {
        if (eventIndex > translationElementIndex) {
            return false;
        }
    ...
    public boolean isFinished() { // TODO add final velocity tolerance
        ...
        boolean rotationAtSetpoint = Math.abs(currentRotationTargetRad
                        .minus(poseSupplier.get().getRotation())
                        .getRadians())
                < Math.toRadians(path.getEndRotationToleranceDeg());
        boolean effectiveRotationAtSetpoint =
                externalRotationAtSetpoint != null ? externalRotationAtSetpoint.getAsBoolean() : rotationAtSetpoint;
        boolean finished = isLastRotationElement
                && isLastTranslationElement
                && translationAtSetpoint
                && effectiveRotationAtSetpoint;
        ...
        logBoolean("FollowPath/finishedRotationAtSetpoint", effectiveRotationAtSetpoint);
        return finished;
    }
    ...
    /**
     * Sets an external rotation-at-setpoint check for isFinished(). When set, isFinished() uses
     * this supplier instead of the path's internal rotation target check. Use when an external
     * system (e.g. SOTM hub rotation) controls the robot's heading.
     */
    public void setExternalRotationAtSetpoint(BooleanSupplier supplier) {
        this.externalRotationAtSetpoint = supplier;
    }
